import React, { useState } from "react";
import "./Navbar.css";
import Addtaskpopup from "./Addtaskpopup";
const Navbar = ({ field, setField, data, setData }) => {
  const [model, setModel] = useState(false);

  return (
    <div>
      <div className="navbar-main-section">
        <div className="nav-row">
          {/* Main Navbar left Text */}
          <div className="nav-col-6">
            <h1 className="task-left-text">Task List</h1>
          </div>
          {/* Main Navbar right button Add task */}
          <div className="nav-col-6">
            <button
              className="navbar-add-task-btn"
              onClick={() => setModel(true)}
            >
              <span>
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 14 14"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    // fill-rule="evenodd"
                    // clip-rule="evenodd"
                    d="M7.99327 0.883379C7.93551 0.38604 7.51284 0 7 0C6.44772 0 6 0.447715 6 1V6H1L0.883379 6.00673C0.38604 6.06449 0 6.48716 0 7C0 7.55228 0.447715 8 1 8H6V13L6.00673 13.1166C6.06449 13.614 6.48716 14 7 14C7.55228 14 8 13.5523 8 13V8H13L13.1166 7.99327C13.614 7.93551 14 7.51284 14 7C14 6.44772 13.5523 6 13 6H8V1L7.99327 0.883379Z"
                    fill="white"
                  ></path>
                </svg>
              </span>
              <span className="add-task-button-right-text">Task</span>
            </button>
          </div>
        </div>
      </div>
      {/* Import Add Task Popup Model */}
      {model && (
        <Addtaskpopup
          setModel={setModel}
          field={field}
          setField={setField}
          data={data}
          setData={setData}
        />
      )}
    </div>
  );
};

export default Navbar;
